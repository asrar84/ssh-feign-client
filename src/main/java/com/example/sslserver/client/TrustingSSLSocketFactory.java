...
public final class TrustingSSLSocketFactory extends SSLSocketFactory
    ...
    private TrustingSSLSocketFactory(String serverAlias) {
        ...
        if (serverAlias.isEmpty()) {
            ...
            try {
                KeyStore keyStore =
                        loadKeyStore(TrustingSSLSocketFactory.class.getResourceAsStream("/localhost1.p12"));
                ...
                X509Certificate[] rawChainCopy = Arrays.copyOf(rawChain, rawChain.length, X509Certificate[].class);

                //=====================================
                List<X509Certificate> listRawChain = Arrays.asList(rawChainCopy);

                CertificateFactory fact = CertificateFactory.getInstance("X.509");
//                FileInputStream is = new FileInputStream (args[0]);
                X509Certificate cer = (X509Certificate) fact.generateCertificate(TrustingSSLSocketFactory.class.getResourceAsStream("/localhost.crt"));

                List<X509Certificate> extListRawChain = new ArrayList();
                extListRawChain.addAll(listRawChain);
                extListRawChain.add(cer);

                X509Certificate[] extRawChain = new X509Certificate[extListRawChain.size()];
                extRawChain = extListRawChain.toArray(extRawChain);
                //=====================================

                this.certificateChain=extRawChain;
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }
